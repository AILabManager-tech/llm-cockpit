...
def test_bound_port_stays_in_allocated_range():
    sock, port = desktop.bind_free_port()
    try:
...
def _occupy_a_port_in_range(sock: socket.socket) -> int:
    ...
    Occupying first and asking afterwards keeps the test independent of
...
def test_bind_free_port_skips_a_busy_port():
...
def test_bind_free_port_raises_when_range_is_exhausted():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as busy:
        taken = _occupy_a_port_in_range(busy)
        with pytest.raises(RuntimeError):
            desktop.bind_free_port(taken, taken)


def test_the_port_is_still_held_when_it_is_handed_back():
    """The whole point: nothing can slip in between reserving and serving.

    The previous version returned a bare number after closing its socket, so
    the port was free again for anyone to take before uvicorn bound it.
    """
    sock, port = desktop.bind_free_port()
    try:
