...
def resolve_in_docs_dir(path: str) -> Path:
    ...
    if resolved != root and root not in resolved.parents:
        raise IngestError(f"chemin hors du dossier autorisé : {path}")
...
def read_document(path: Path) -> str:
    ...
    if suffix == ".pdf":
        try:
            ...
            raise IngestError("pypdf indisponible pour lire le PDF") from exc
        try:
            reader = PdfReader(str(path))
            return "\n".join((page.extract_text() or "") for page in reader.pages)
        except Exception as exc:  # noqa: BLE001 - pdf illisible
            raise IngestError(f"PDF non parsable : {path.name} ({exc})") from exc
    raise IngestError(f"type de fichier non supporté : {suffix}")
...
def chunk_text(
    ...
    if size <= 0:
        raise IngestError("taille de chunk invalide")
...
async def ingest_document(path_str: str) -> RagDocument:
    ...
    if not chunks:
        raise IngestError(f"document vide après extraction : {path.name}")
